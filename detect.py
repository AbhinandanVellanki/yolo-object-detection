...
def is_hazard(bounding_box, droi_polygon):
    """
    ...
    """
    top_left, bottom_right = (int(bounding_box[0]), int(bounding_box[1])), (int(bounding_box[2]), int(bounding_box[3]))
    bottom_left = (top_left[0], bottom_right[1])
    top_right = (bottom_right[0], top_left[1])
    return droi_polygon.contains(Point(top_left)) or droi_polygon.contains(Point(top_right)) or droi_polygon.contains(Point(bottom_right)) or droi_polygon.contains(Point(bottom_left))

def get_colour(hazard):
    global blue, yellow, green, orange, red, white
    if hazard:
        return red
    else:
        return yellow
...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    global detection_boundary, red, white, yellow, green, blue, orange
    ...
    print("To set detection boundary, click on four corners and press any key")
    img  = dataset[0]
    ...
    while True: 
        print("Capturing detection boundary.",len(detection_boundary), "clicks captured. Press \'q\' to stop capturing")
    ...
    detection_polygon = Polygon(detection_boundary)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        hazard = is_hazard(xyxy, detection_polygon)
                        plot_one_box(xyxy, im0, label=label, color=get_colour(hazard), line_thickness=line_thickness)
                        if hazard:
                            hazards += 1
                        else:
                            potential_hazards += 1
            ...
            im0 = draw_roi(im0, detection_boundary, red)

            # write number of hazards and number of potential hazards
            cv2.putText(im0, "Hazards: "+str(hazards), (int(width - 0.40*width), int(height-0.1*height)), cv2.FONT_HERSHEY_SIMPLEX, 1, blue, 2)
            cv2.putText(im0, "Potential Hazards: "+str(potential_hazards), (int(width - 0.40*width), int(height- 0.05*height)), cv2.FONT_HERSHEY_SIMPLEX, 1, blue, 2)
